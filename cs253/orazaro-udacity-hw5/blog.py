...
class Post(db.Model):
    ...
    def json(self):
        j = ''
        j += '{"subject":"%s",' % self.subject.replace('"','\\"') 
        j += '"content":"%s",' % self.content.replace('"','\\"') 
        j += '"created":"%s",' % self.created.strftime("%a %b %d %H:%M:%S %Y") 
        j += '"last_modified":"%s"}' % self.last_modified.strftime("%a %b %d %H:%M:%S %Y")
        j = j.replace('\n','\\n')
        j = j.replace('\r','\\r')
        return j
